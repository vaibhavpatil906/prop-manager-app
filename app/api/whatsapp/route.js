...
export async function POST(req) {
  try {
    ...
    // 2. Global Reset / Menu
    if (['hi', 'menu', 'start', 'cancel', 'back', 'main menu'].includes(input)) {
      await clearSession(from)
      await sendListMenu(from, 
        `👋 PropManager Home`,
        "Select an action to manage your properties:",
        "Main Menu",
        [
          { title: "⚡ RECORD", rows: [{ id: "path_reading", title: "Submit Reading", description: "Step-by-step entry" }] },
          { title: "📊 REPORTS", rows: [
              { id: "path_monthly", title: "Monthly Report", description: "Property-wide breakdown" },
              { id: "path_unpaid", title: "Unpaid Bills", description: "Outstanding by month" }
          ]},
          { title: "🔍 LOOKUP", rows: [
              { id: "path_lookup", title: "Get Unit Bill", description: "Search by Tenant" },
    ...
    // 3. Handle Session Steps (If active)
    if (session) {
      // SUBMIT READING FLOW
      if (session.step === 'awaiting_unit_reading') {
        ...
        if (!unit?.tenants?.[0]) return await sendText(from, `❌ Unit *${unitNum}* not found. Try again or type 'Menu'.`)
      ...
      if (session.step === 'awaiting_water_value') {
        if (input === 'enter custom') return await sendText(from, "Type water bill amount:")
        ...
        await supabase.from('utility_bills').upsert({
          user_id: profile.id, tenant_id: session.tenant_id, billing_month: monthCode,
        ...
        await sendButtons(from, summary, ["Main Menu", "Submit Reading"])
      ...
      if (session.step === 'awaiting_tenant_selection') {
        ...
        if (!tenantId) return await sendText(from, "❌ Error. Use the menu or type 'Menu'.")
        ...
        if (!bills?.length) {
          await sendButtons(from, `📭 No bill history for *${tenant?.name}*.`, ["Main Menu", "Get Unit Bill"])
          return await clearSession(from)
        }
        ...
        return await sendListMenu(from, `📅 Bills: ${tenant?.name}`, "Select a month:", "Select Month", [{ title: "MONTHS", rows: bills.map(b => ({ id: `month_${b.billing_month}`, title: b.billing_month })) }])
      }
      ...
      if (session.step === 'awaiting_month_selection') {
        ...
        if (bill) {
          ...
          await sendButtons(from, billMsg, ["Main Menu", "Get Unit Bill"])
        }
        return await clearSession(from)
      }
    ...
    if (listId === 'path_reading' || input === 'submit reading') {
      ...
      return await sendListMenu(from, "🔍 Select Tenant", "Choose a tenant to view history:", "Select", [{ title: "ACTIVE TENANTS", rows: tenants.map(t => ({ id: `tenant_${t.id}`, title: `${uMap[t.unit_id] || 'Unit'} - ${t.name}`.substring(0, 24) })) }])
    }
      ...
      const msg = props?.length ? `🏢 *Your Properties:*\n` + props.map(p => `• ${p.name}: ${p.units} units`).join('\n') : "🏠 No properties found."
      return await sendButtons(from, msg, ["Main Menu"])
    }
      ...
      if (!bills?.length) return await sendButtons(from, "✅ No unpaid bills found!", ["Main Menu"])
      ...
      return await sendButtons(from, r + `⭐ *TOTAL DUE: ₹${gt.toLocaleString()}*`, ["Main Menu"])
    }

    await sendText(from, "❓ Send *Hi* for the menu.")
    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error('SERVER ERROR:', err); return NextResponse.json({ ok: true })
  }
}
